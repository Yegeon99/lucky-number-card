...
type Hero = { image: string; artist: string; album: string };
    ...
    <div className="phone flex flex-col">
      ...
      <header className="relative overflow-hidden">
        <div
          ...
          style={{
            ...
            backgroundColor: "#a8141a",
          }}
          aria-hidden
        />
        <div
          className="absolute inset-0 bg-gradient-to-b from-black/10 via-transparent to-paper"
          aria-hidden
        />
        ...
        <div className="relative px-6 pb-8 pt-7">
          ...
          <motion.div
            ...
            className="mt-9"
          >
            <h1 className="font-wide text-[50px] leading-[0.86] tracking-[-0.02em] text-black">
              CHOOM
            </h1>
          </motion.div>
      ...
      <main className="px-6 pb-[112px] pt-4">
        ...
        <p className="mt-8 text-center text-[10.5px] leading-relaxed text-ink-3/70">
          {footer}
          <br />
          이미지는 앨범 공식 공개 화보이며 데모 시연용입니다. 실제 상품 사용은
          기획사 승인이 필요합니다.
        </p>
      </main>
...
function PhotoCard({ item, showHint }: { item: Item; showHint: boolean }) {
  return (
    <Link
      href={item.href}
      className="block"
      aria-label={`${item.member} No. ${item.serial}`}
    >
      <motion.div
        whileTap={{ scale: 0.96 }}
        transition={{ type: "spring", stiffness: 500, damping: 30 }}
      ...
      >
        ...
        {showHint && item.hint && (
          <span
            className={`absolute right-2.5 top-2.5 z-[4] h-2.5 w-2.5 rounded-full ${item.hint === "lucky" ? "dot-gold" : "dot-silver"}`}
          />
        )}

        {/* 머그샷 이름판: 관리자 모드에서만 (멤버·번호 확인용) */}
        {showHint && (
          <div className="absolute inset-x-2.5 bottom-2.5 z-[4] rotate-[-1.5deg] rounded-[3px] bg-placard/92 px-2.5 py-2 text-white shadow-[0_6px_14px_-6px_rgba(0,0,0,0.6)]">
            <p className="font-narrow text-[7px] uppercase tracking-[0.18em] text-white/75">
              Department of Choom
            </p>
            <p className="font-narrow mt-0.5 text-[17px] font-bold leading-none tracking-[0.02em]">
              {item.nameEn}
            </p>
